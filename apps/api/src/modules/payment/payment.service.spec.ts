...
describe('PaymentService', () => {
  ...
  beforeEach(async () => {
    ...
    const stubRegistry = new PaymentGatewayRegistry();
    // Register the same mock so resolveGateway('stripe' | 'instapay' | etc.)
    // delegates to the spy. We don't register by name — resolveGateway falls
    // back to the default gateway (PAYMENT_GATEWAY) when the provider isn't
    // in the registry, which matches existing test expectations.
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: PaymentGatewayRegistry, useValue: stubRegistry },
      ],
    }).compile();
    ...
    it('should call gateway and create authorized payment', async () => {
      ...
      const module: TestingModule = await Test.createTestingModule({
        providers: [
          ...
          { provide: PAYMENT_GATEWAY, useValue: mockGateway },
          { provide: WebhookService, useValue: mockWebhookService },
          { provide: PaymentGatewayRegistry, useValue: new PaymentGatewayRegistry() },
        ],
      }).compile();
      ...
      expect(mockGateway.authorize).toHaveBeenCalledWith(
        'tok_test_123',
        500,
        'USD',
        { propertyId: 'prop-001' },
