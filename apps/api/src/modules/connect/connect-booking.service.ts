...
import { PropertyPaymentSettingsService } from '../payment/property-payment-settings.service';
...
export class ConnectBookingService {
  private readonly logger = new Logger(ConnectBookingService.name);
  ...
  constructor(
    ...
    private readonly propertyPaymentSettings: PropertyPaymentSettingsService,
  ) {}
  ...
  async book(dto: AgentBookDto) {
    ...
    let paymentStatus: 'none' | 'authorized' | 'charged' | 'pending' = 'none';
    let depositAmount: number | undefined;
    let paymentRedirectUrl: string | undefined;
    let paymentId: string | undefined;

    if (dto.paymentMethod === 'instapay') {
      // Real authorization through the Instapay (Egyptian IPN) gateway —
      // creates a folio first, then issues a pre-auth that may return a
      // bank-app redirect URL the booking engine hands off to the guest.
      if (!dto.paymentToken) {
        throw new BadRequestException(
          'paymentToken is required for instapay (payer alias / IPN reference)',
        );
      }
      // Confirm Instapay is enabled on this property before we charge anything.
      await this.propertyPaymentSettings.getInstapayConfig(dto.propertyId);
      ...
      const authorized = await this.paymentService.authorizePayment({
        folioId: folio.id,
        propertyId: dto.propertyId,
        amount: totalAmountDec.toFixed(2),
        currencyCode: ratePlan.currencyCode,
        gatewayProvider: dto.paymentProvider ?? 'instapay',
        gatewayPaymentToken: dto.paymentToken,
      } as any);

      paymentStatus = (authorized as any).redirectUrl ? 'pending' : 'authorized';
      depositAmount = totalAmount;
      paymentRedirectUrl = (authorized as any).redirectUrl;
      paymentId = (authorized as any).id;
    } else if (dto.paymentMethod === 'prepaid' || dto.paymentMethod === 'virtual_card') {
