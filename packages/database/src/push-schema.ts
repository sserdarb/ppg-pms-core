...
async function main() {
  ...
  const enums = [
    ...
    `DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'payment_method') THEN CREATE TYPE payment_method AS ENUM ('credit_card','debit_card','cash','bank_transfer','city_ledger','vcc','instapay','other'); END IF; END $$`,
    // Idempotent add: append instapay to payment_method if it already existed without it
    `DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_enum e JOIN pg_type t ON t.oid = e.enumtypid WHERE t.typname = 'payment_method' AND e.enumlabel = 'instapay') THEN ALTER TYPE payment_method ADD VALUE 'instapay'; END IF; END $$`,
  ...
  const alters = [
    ...
    `ALTER TABLE properties ADD COLUMN IF NOT EXISTS external_id varchar(100)`,
    `ALTER TABLE properties ADD COLUMN IF NOT EXISTS provisioned_by varchar(50)`,
    `ALTER TABLE properties ADD COLUMN IF NOT EXISTS provisioned_at timestamptz`,
    `CREATE UNIQUE INDEX IF NOT EXISTS properties_external_id_uniq ON properties(external_id) WHERE external_id IS NOT NULL`,
  ];
